"""Obsidian promotion pipeline — Steps 3-5 of the Research Knowledge Pipeline.
...
def propose_note_content(summary: KnowledgeSummary) -> str:
    ...
    Produces a document with YAML frontmatter, a level-1 title, summary text,
    conclusions, topic blocks with confidence-tagged findings, source links,
    entity wikilinks, knowledge gaps, and a metadata section.
    """
    lines: list[str] = []
...
def promote_to_obsidian(
    ...
    promoted_proposal = proposal._replace(
        state=CurationGateState.VAULTED,
        vault_path=str(dest),
        updated_at=now,
    )
    ...
    return {
        ...
        "vaulted_proposal": promoted_proposal,
    }
