"""Domain-layer goal execution planning engine.

This module is the behavioral core of the goal execution planning
extension — the rules-based next-action engine, dynamic task-to-milestone
derivation, and the dict-to-model reconstruction helpers that several
services previously duplicated.

It lives in :mod:`janus.domain` because it depends **only** on the model
layer (:mod:`janus.models`) — no integrations or services.  This removes
the layering violation where pure domain logic was trapped in the
services package (``services/next_action.py``) and copied into
``services/attention.py``.

Public API:
    NextAction
    derive_next_action(goal, tasks, completed_task_titles, today, projects=None)
    derive_milestone_tasks(milestone, all_milestones, goal, open_task_titles)
    derive_milestone_task_set(milestones, goal, open_task_titles)
    milestone_objs(goal)
    project_objs(goal)
"""
...
def project_objs(goal: Goal) -> list[Project]:
...
def derive_milestone_tasks(
    ...
    * All open tasks from ``goal.related_tasks`` that belong to the
    ...
    If *milestone* is not the earliest non-terminal milestone, no tasks
    are assigned to it (they belong to the current active milestone instead).
...
def derive_next_action(
    ...
    if not projects:
        ...
        current_ms = _first_active_milestone(milestone_objs_list)
        if current_ms is not None:
            current_ms_tasks = derive_milestone_task_set(
                milestone_objs_list, goal, open_titles
            )
    ...
    if current_ms is not None:
        start_idx = None
        for i, m in enumerate(milestone_objs_list):
            if m.order == current_ms.order:
                start_idx = i + 1
                break
        if start_idx is not None:
            for m in milestone_objs_list[start_idx:]:
    ...
    all_milestone = milestone_objs_list
    if current_ms is not None:
        ...
        for i, m in enumerate(milestone_objs_list):
            if m.order == current_ms.order:
                start_idx = i + 1
                break
        search_range = milestone_objs_list[start_idx:] if start_idx else []
    else:
        search_range = milestone_objs_list
